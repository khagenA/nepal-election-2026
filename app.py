...
# ── Official Nepal party brand colors ─────────────────────────────────────────
PARTY_COLORS = [
    ...
    ('#388E3C', ['समाजवादी', 'जनता']),       # JSP   — green
...
def mobile_layout(title_text, center_text, height):
    return dict(
        ...
        margin=dict(t=100, b=170, l=10, r=10),
        height=height, paper_bgcolor='#FAFAFA', autosize=True,
    )
...
def build_charts(parties, votes):
    ...
    fig1 = go.Figure(go.Pie(
        ...
        marker=dict(colors=chart1_df['Color'].tolist(), line=dict(color='white', width=2)),
    ...
    fig2 = go.Figure(go.Pie(
        ...
        texttemplate="<b>%{value}</b>", textposition='inside',
        insidetextorientation='radial',
        marker=dict(colors=seats_df['Color'].tolist(), line=dict(color='white', width=2)),
        pull=[0.06 if i == 0 else 0 for i in range(len(seats_df))],
        sort=False, direction='clockwise', rotation=90,
    ))
    fig2.update_layout(**mobile_layout(
        title_text=(f"<b>110 PR Seat Allocation</b><br>"
                    f"<sup>Parties ≥ 3% &nbsp;|&nbsp; Largest Remainder &nbsp;|&nbsp; {timestamp}</sup>"),
        center_text=f"<b>{TOTAL_PR_SEATS}</b><br>PR Seats", height=580,
    ))
...
.gradio-container { max-width: 1000px !important; margin: 0 auto !important; padding: 8px !important; }
#title { text-align:center; color:#1a237e; font-size:clamp(1.1rem,4vw,1.5rem);
         font-weight:bold; margin-bottom:2px; line-height:1.3; }
#subtitle { text-align:center; color:#555; font-size:clamp(0.75rem,2.5vw,0.88rem);
            margin-bottom:10px; line-height:1.4; }
#disclaimer { background:#FFF8E1; border-left:5px solid #F9A825; border-radius:6px;
              padding:10px 14px; margin:8px 0 14px 0;
              font-size:clamp(0.75rem,2.5vw,0.85rem); color:#5D4037; line-height:1.6; }
#disclaimer strong { color:#E65100; }
#disclaimer a { color:#1565C0; font-weight:bold; }
#table-title { font-size:clamp(1rem,3vw,1.15rem); font-weight:bold; color:#1a237e;
               margin:16px 0 6px 0; padding-bottom:4px; border-bottom:2px solid #1a237e; }
#charts-title { font-size:clamp(1rem,3vw,1.15rem); font-weight:bold; color:#1a237e;
                margin:20px 0 8px 0; padding-bottom:4px; border-bottom:2px solid #1a237e; }
#status textarea { background:#E8F5E9 !important; color:#1B5E20 !important;
                   border-left:4px solid #2E7D32 !important; font-size:0.82rem !important; }
.js-plotly-plot, .plotly, .plot-container { width:100% !important; }
#footer { text-align:center; color:#aaa; font-size:clamp(0.7rem,2vw,0.75rem);
          margin-top:14px; line-height:1.6; }
@media (min-width:600px) { .gradio-container { padding:16px !important; } }
"""
...
with gr.Blocks(title="Nepal Election 2082 — PR Results") as demo:
    ...
    gr.HTML("""
        <div id='disclaimer'>
            ...
            political party. Data may be delayed or incomplete.<br>
    ...
    gr.HTML("<div id='table-title'>📋 Party Results at a Glance</div>")
    table_out = gr.DataFrame(
        headers=['Party', 'Votes', 'Vote %', 'PR Seats (110)'],
        interactive=False, wrap=True,
    )

    gr.HTML("<div id='charts-title'>📊 Interactive Charts</div>")
    with gr.Tabs():
